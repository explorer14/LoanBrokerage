...
namespace LoanRequestSender
{
    public class Functions
    {
        ...
        public Functions(ILoanRequestSenderFilter loanRequestSenderFilter,
            IPipe<LoanQuoteResponse> pipe,
            ILogger logger)
        {
            ...
            TextWriter log)
        {
            ...
            logger.Information(
                "Now sending the loan quote {@request} to all the registered banks...",
                loanQuoteRequest);
            var quoteResponse = await loanRequestSenderFilter
                .GetLoanQuotesFromRegisteredBanks(loanQuoteRequest);
