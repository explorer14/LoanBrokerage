...
namespace CustomerNotifier.Filter
{
    public class CustomerNotifierFilter : ICustomerNotifierFilter
    {
        ...
        public async Task NotifyCustomer(
            CustomerLoanQuote customerLoanQuote)
        {
            if (customerLoanQuote == null)
                throw new ArgumentNullException(
                    $"{nameof(customerLoanQuote)} is null!");

            if (customerLoanQuote?.Quotes == null)
                throw new ArgumentNullException(
                    $"{nameof(CustomerLoanQuote.Quotes)} is null!");

            logger.Information("Retrieving customer information from the database...");
            var customerInfo = await customerRepository
                .CustomerByBSN(
                customerLoanQuote.BSN);
            await notificationService
                .Notify(
                customerInfo.Email,
                JsonConvert.SerializeObject(
                    customerLoanQuote.Quotes));
        }
    }
}